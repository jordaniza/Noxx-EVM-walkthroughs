...
contract WETHStorage {
    ...
    function getStorageDynamic(uint256 _slot, uint256 _offset) external view returns (bytes32 result) {
...
contract RunWeth is Script {
    function run() public {
        ...
        uint256 stringLength = uint256(weth.getStorageAt(3));
        ...
        for (uint256 i; i < (stringLength / 64) + 1; i++) {
